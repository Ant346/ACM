...
class Controller(ABC):
    ...
    def compute_action_sampled(
        ...
        return self.actor.action if hasattr(self, "actor") else np.ones(2)
...
class Controller3WRobotNIDisassembledCLF:
    ...
    def __init__(
        self, controller_gain=0.1, action_bounds=None, time_start=0, sampling_time=0.1
    ):
        self.controller_gain = controller_gain
        self.action_bounds = action_bounds
        self.controller_clock = time_start
...
class NominalControllerInvertedPendulum:
    def __init__(
        ...
        self.action_bounds = action_bounds
        self.controller_gain = controller_gain
        self.observatiobservationtime = sampling_time
        self.action = np.array([np.mean(action_bounds)])
...
class Controller3WRobotNIMotionPrimitive:
    ...
    def compute_action_sampled(self, state, observation, time, observation_target=[]):
        """
        Compute sampled action.
...
class Controller3WRobotClfSemiconcave:
    ...
    def compute_LF(self, state):
        state_NH = self._Cart2NH(state)

        LF_value = (
            rc.sum_2(state_NH)
            + state_NH[2] ** 2
            + rc.abs(state_NH[2])
            * (10 - 2 * (rc.abs(state_NH[0]) + rc.abs(state_NH[1])))
        )
    ...
    def compute_LF(self, state):
        ...
        LF_value = (
            rc.sum_2(state_NH)
            + state_NH[2] ** 2
            - 2 * rc.abs(state_NH[2]) * rc.sqrt(state_NH[0] ** 2 + state_NH[1] ** 2)
        )
    ...
    def compute_action(self, state, observation, time=0, observation_target=[]):
        ...
        objective = lambda action: self.compute_LF(
            self.predictor.predict(observation, action)
        )
        self.action = self.optimizer.optimize(
            objective, self.action, self.action_bounds
        )
        self.predicted = self.predictor.predict(observation, self.action)
